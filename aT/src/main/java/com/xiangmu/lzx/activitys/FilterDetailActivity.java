...
public class FilterDetailActivity extends AppCompatActivity implements OnItemSelectedListener{
    Context context =FilterDetailActivity.this;
...
private HttpHandler<String> handler;
    ...
    @InjectView(R.id.filter_id) TextView filter_id;
    @InjectView(R.id.filter_filters)   MaterialEditText filter_filters;
    @InjectView(R.id.filter_createdate) TextView filter_createdate;
...
//                Toast.makeText(this, "单击了添加按钮", Toast.LENGTH_SHORT).show();
          //      new AlertDialog.Builder(FilterListEditActivity.this).setMessage("请选择类型！！").setPositiveButton("确定", null).show();
               url=xinWenURL.getRepareFilterMob()+filterEntity.getId();
                SaveData(url);
            }
        });
    }
    ...
    if (!url.equals("")) {
        ...
        handler = httpUtils.send(HttpRequest.HttpMethod.GET, url, params,new RequestCallBack<String>() {
            ...
            public void onSuccess(ResponseInfo<String> responseInfo) {
                ...
                if (responseInfo.result != null) {
                    Toast.makeText(FilterDetailActivity.this, "过滤词语修改成功！", Toast.LENGTH_SHORT).show();
...
//                        setResult(RESULT_CODE, intent);
            ...
            @Override
            public void onFailure(HttpException e, String s) {
                Toast.makeText(FilterDetailActivity.this, "过滤词语修改失败！", Toast.LENGTH_SHORT).show();
            }
        });
