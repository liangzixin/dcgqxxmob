...
public class LiuyuanListEditActivity extends AppCompatActivity implements MyItemClickListener, MyItemLongClickListener, OnDismissListener, OnItemClickListener {
...
//    private GridView gv_searchHistory;
    ...
    private SearchProductArticlerAdapter searchProductArticlerAdapter;
    private MySqlitehelper mySqlitehelper;
    private SQLiteDatabase writableDatabase;
    private List<XinWen_productinfo.T18908805728Entity.AdsEntity> listads;//字段listads
    private List<XinWen_productinfo.ProductArticlerEntity> liuyuan_list=new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_liuyuansearchedit);
    ...
    private void inintClick() {
        back.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                LiuyuanListEditActivity.this.finish();
                //    finish();
            }
        });
    ...
    private void getshowdata(String data, boolean refresh) {
     //   customer_list.clear();
        ...
        if ( searchProductArticlerAdapter  == null||refresh) {//在数据之后adapter之前增加轮播才会不anr
            ...
            searchProductArticlerAdapter.setToutiao_list(liuyuan_list);//填充并刷新数据
        }
        searchProductArticlerAdapter.setOnItemClickListener(new SearchProductArticlerAdapter.MyItemClickListener() {
            @Override
...
//                liuyuan_list.addAll(toutiao_object.getT18908805728());
//
...
//                layoutsearchResult.setVisibility(View.VISIBLE);//显示搜索结果布局
//
//                searchcustomerAdapter = new SearchCustomerAdapter(liuyuan_list, this);
//
...
//            inintAdapter();
    ...
    //跳转详细页面方法
    @SuppressLint("NewApi")
    private void frament2activity(int position, List<XinWen_productinfo.ProductArticlerEntity> liuyuan_list) {
        int pos;
        ...
        //    int bujutype = XinWen_adapter.getType(liuyuan_list.get(pos).getId());
...
//        LogUtils.e("xinwenadapter", "type==" + bujutype);
        ...
        xinWenXi.setId(liuyuan_list.get(pos).getId());
        int bujutype = 0;
//        bujutype = liuyuan_list.get(pos).getLanmu();
//        xinWenXi.setBujuType(bujutype);
////        xinWenXi.setLanMuType(daohangtype);
//        xinWenXi.setLanMuType(1);
//        xinWenXi.setReplaycount(liuyuan_list.get(pos).getArticlers().size());//跟帖数量
//        xinWenXi.setTitle(liuyuan_list.get(pos).getName());//标题
//        xinWenXi.setXinwentext(liuyuan_list.get(pos).getDescription());//内容
//        xinWenXi.setCreateDate(liuyuan_list.get(pos).getCreateTime());//日期
//        xinWenXi.setGsmz(liuyuan_list.get(pos).getGsmz());
//        xinWenXi.setGsdz(liuyuan_list.get(pos).getGsdz());
//        xinWenXi.setLxr(liuyuan_list.get(pos).getLxr());
//        xinWenXi.setLxdh(liuyuan_list.get(pos).getLxdh());
//        xinWenXi.setZpxx(liuyuan_list.get(pos).getZpxx());
//        xinWenXi.setFwcs(liuyuan_list.get(pos).getFwcs());
//        xinWenXi.setGqxx(liuyuan_list.get(pos).getGqxx());
//        xinWenXi.setProductCategory(liuyuan_list.get(pos).getProductcategory());
////xinWenXi.setUploadFiles(liuyuan_list.get(pos).getUploadFile());
//        for (int i = 0; i < liuyuan_list.get(pos).getUploadFile().size(); i++) {
//            UploadFile uploadFile = new UploadFile();
//
//            uploadFile.setPath(liuyuan_list.get(pos).getUploadFile().get(i).getPath());
//            potolist.add(uploadFile);
//        }
//        xinWenXi.setUploadFileList(potolist);
//        for (int i = 0; i < liuyuan_list.get(pos).getProductArticler().size(); i++) {
//            ProductArticler productArticler = new ProductArticler();
//            productArticler.setArtreview_authorid(liuyuan_list.get(pos).getProductArticler().get(i).getArtreview_authorid());
//            productArticler.setArtreview_time(liuyuan_list.get(pos).getProductArticler().get(i).getArtreview_time());
//            productArticler.setArtreview_content(liuyuan_list.get(pos).getProductArticler().get(i).getArtreview_content());
...
//        }
        ...
        switch (bujutype) {
            ...
            case XinWen_adapter.TYPE_zhibo:
                ...
                //  String urlzhibo = liuyuan_list.get(pos).getUrl();
                // String urlzhibo = liuyuan_list.get(pos).getName();
...
//                //   String urlduotuRight = liuyuan_list.get(pos).getId();
//                String urlduotuRight = liuyuan_list.get(pos).getName();
...
//                String urlduotu = "http://c.3g.163.com/photo/api/set/" + urlRight + ".json";
  ...
  //  @Override
    public void onItemClick1(View view, int postion) {
        bean = new XinWen_productinfo.ProductArticlerEntity();
        bean = liuyuan_list.get(postion);
        id = bean.getId();
        switch (view.getId()) {
            case R.id.result_title://详细信息
                ...
                //    l = 1;
                frament2activity(postion, liuyuan_list);
                break;
            ...
            case R.id.result_agree://审核
                mAlertView2 = new AlertView("审核",bean.getArtreview_content().substring(0,6), "取消", new String[]{"确定"}, null, this, AlertView.Style.Alert, this).setCancelable(true).setOnDismissListener(this);
                mAlertView2.show();
                break;
        }
            ...
            handler = httpUtils.send(HttpRequest.HttpMethod.GET, url, new RequestCallBack<String>() {
                ...
                public void onSuccess(ResponseInfo<String> responseInfo) {
                    if (responseInfo.result.equals("true")) {
                        Toast.makeText(LiuyuanListEditActivity.this, "删除成功", Toast.LENGTH_SHORT).show();
                ...
                @Override
                public void onFailure(HttpException e, String s) {
                    Toast.makeText(LiuyuanListEditActivity.this, "删除失败", Toast.LENGTH_SHORT).show();
                }
            });
    ...
    private void AgreeData(final String url) {
        if (!url.equals("")) {
            ...
            handler = httpUtils.send(HttpRequest.HttpMethod.GET, url, new RequestCallBack<String>() {
                ...
                public void onSuccess(ResponseInfo<String> responseInfo) {
                    if (responseInfo.result.equals("true")) {
                        Toast.makeText(LiuyuanListEditActivity.this, "审核成功", Toast.LENGTH_SHORT).show();
                        isPause = true;
                        onResume();
                    }
                ...
                @Override
                public void onFailure(HttpException e, String s) {
                    Toast.makeText(LiuyuanListEditActivity.this, "审核失败", Toast.LENGTH_SHORT).show();
                }
            });
