...
public class SmsListEditActivity extends AppCompatActivity implements MyItemClickListener, MyItemLongClickListener, OnDismissListener, OnItemClickListener {
...
//    private GridView gv_searchHistory;
    ...
    private List<XinWen_productinfo.ShortmessageEntity> shortmessage_list=new ArrayList<>();
    private XinWen_productinfo.ShortmessageEntity bean;
   ...
   // private SimpleArcDialog mDialog;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bindViews();
        url = xinWenURL.getShortmessage();//最新url
    ...
    private void inintClick() {
        back.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                SmsListEditActivity.this.finish();
                //    finish();
            }
        });
    ...
    private void getshowdata(String data, boolean refresh) {
     //   customer_list.clear();
        if (refresh) {
            ...
            shortmessage_list.clear();
        }

        XinWen_productinfo productArticler_object = XinWenproductinfoJson.getdataShortmessage(data);//传入类型和数据
        shortmessage_list.addAll(productArticler_object.getListShortmessageEntity());
        searchjiekuo.setText("共: " + productArticler_object.getTotalRecords() + " 条");
        Collections.sort(shortmessage_list, new Comparator<XinWen_productinfo.ShortmessageEntity>() {
            public int compare(XinWen_productinfo.ShortmessageEntity arg0, XinWen_productinfo.ShortmessageEntity arg1) {
                return arg1.getId().compareTo(arg0.getId());
            }
        });
        layoutsearchResult.setVisibility(View.VISIBLE);//显示搜索结果布局
        if ( smsAdapter  == null||refresh) {//在数据之后adapter之前增加轮播才会不anr
            smsAdapter = new SmsAdapter(shortmessage_list, this);
            lv_searchResult.getRefreshableView().setAdapter(smsAdapter);
        }else {
            smsAdapter.setToutiao_list(shortmessage_list);//填充并刷新数据
        }
        smsAdapter.setOnItemClickListener(new SmsAdapter.MyItemClickListener() {
            @Override
...
//                shortmessage_list.addAll(toutiao_object.getT18908805728());
//
...
//                layoutsearchResult.setVisibility(View.VISIBLE);//显示搜索结果布局
//
//                searchcustomerAdapter = new SearchCustomerAdapter(shortmessage_list, this);
//
...
//            inintAdapter();
    ...
    //跳转详细页面方法
    @SuppressLint("NewApi")
    private void frament2activity(int position, List<XinWen_productinfo.ShortmessageEntity> shortmessage_list) {
        int pos;
        ...
        //    int bujutype = XinWen_adapter.getType(shortmessage_list.get(pos).getId());
...
//        LogUtils.e("xinwenadapter", "type==" + bujutype);
        ...
        int bujutype = 0;
//        bujutype = shortmessage_list.get(pos).getLanmu();
//        xinWenXi.setBujuType(bujutype);
////        xinWenXi.setLanMuType(daohangtype);
//        xinWenXi.setLanMuType(1);
//        xinWenXi.setReplaycount(shortmessage_list.get(pos).getArticlers().size());//跟帖数量
//        xinWenXi.setTitle(shortmessage_list.get(pos).getName());//标题
//        xinWenXi.setXinwentext(shortmessage_list.get(pos).getDescription());//内容
//        xinWenXi.setCreateDate(shortmessage_list.get(pos).getCreateTime());//日期
//        xinWenXi.setGsmz(shortmessage_list.get(pos).getGsmz());
//        xinWenXi.setGsdz(shortmessage_list.get(pos).getGsdz());
//        xinWenXi.setLxr(shortmessage_list.get(pos).getLxr());
//        xinWenXi.setLxdh(shortmessage_list.get(pos).getLxdh());
//        xinWenXi.setZpxx(shortmessage_list.get(pos).getZpxx());
//        xinWenXi.setFwcs(shortmessage_list.get(pos).getFwcs());
//        xinWenXi.setGqxx(shortmessage_list.get(pos).getGqxx());
//        xinWenXi.setProductCategory(shortmessage_list.get(pos).getProductcategory());
////xinWenXi.setUploadFiles(shortmessage_list.get(pos).getUploadFile());
//        for (int i = 0; i < shortmessage_list.get(pos).getUploadFile().size(); i++) {
//            UploadFile uploadFile = new UploadFile();
//
//            uploadFile.setPath(shortmessage_list.get(pos).getUploadFile().get(i).getPath());
//            potolist.add(uploadFile);
//        }
//        xinWenXi.setUploadFileList(potolist);
//        for (int i = 0; i < shortmessage_list.get(pos).getProductArticler().size(); i++) {
//            ProductArticler productArticler = new ProductArticler();
//            productArticler.setArtreview_authorid(shortmessage_list.get(pos).getProductArticler().get(i).getArtreview_authorid());
//            productArticler.setArtreview_time(shortmessage_list.get(pos).getProductArticler().get(i).getArtreview_time());
//            productArticler.setArtreview_content(shortmessage_list.get(pos).getProductArticler().get(i).getArtreview_content());
...
//        }
        ...
        switch (bujutype) {
            ...
            case XinWen_adapter.TYPE_zhibo:
                ...
                //  String urlzhibo = shortmessage_list.get(pos).getUrl();
                // String urlzhibo = shortmessage_list.get(pos).getName();
...
//                //   String urlduotuRight = shortmessage_list.get(pos).getId();
//                String urlduotuRight = shortmessage_list.get(pos).getName();
...
//                String urlduotu = "http://c.3g.163.com/photo/api/set/" + urlRight + ".json";
  ...
  //  @Override
    public void onItemClick1(View view, int postion) {
        bean = new XinWen_productinfo.ShortmessageEntity();
        bean = shortmessage_list.get(postion);
        id = bean.getId();
        switch (view.getId()) {
            case R.id.result_title://详细信息
                ...
                //    l = 1;
                frament2activity(postion, shortmessage_list);
                break;
            ...
            case R.id.bt_delete://删除
                ...
                mAlertView1 = new AlertView("删除",bean.getMobile(), "取消", new String[]{"确定"}, null, this, AlertView.Style.Alert, this).setCancelable(true).setOnDismissListener(this);
                mAlertView1.show();
                break;
            case R.id.result_agree://审核
                mAlertView2 = new AlertView("审核",bean.getMobile().substring(0,6), "取消", new String[]{"确定"}, null, this, AlertView.Style.Alert, this).setCancelable(true).setOnDismissListener(this);
                mAlertView2.show();
                break;
        }
...
//    }
    ...
    public void onItemClick(Object o, int position) {
        ...
        if (mAlertView1 == mAlertView0) {
            String clickdel = xinWenURL.getClickShortmessagedel() + bean.getId();
            DelData(clickdel);
            ...
            handler = httpUtils.send(HttpRequest.HttpMethod.GET, url, new RequestCallBack<String>() {
                ...
                public void onSuccess(ResponseInfo<String> responseInfo) {
                    if (responseInfo.result.equals("true")) {
                        Toast.makeText(SmsListEditActivity.this, "删除成功", Toast.LENGTH_SHORT).show();
                ...
                @Override
                public void onFailure(HttpException e, String s) {
                    Toast.makeText(SmsListEditActivity.this, "删除失败", Toast.LENGTH_SHORT).show();
                }
            });
    ...
    private void AgreeData(final String url) {
        if (!url.equals("")) {
            ...
            handler = httpUtils.send(HttpRequest.HttpMethod.GET, url, new RequestCallBack<String>() {
                ...
                public void onSuccess(ResponseInfo<String> responseInfo) {
                    if (responseInfo.result.equals("true")) {
                        Toast.makeText(SmsListEditActivity.this, "审核成功", Toast.LENGTH_SHORT).show();
                        isPause = true;
                        onResume();
                    }
                ...
                @Override
                public void onFailure(HttpException e, String s) {
                    Toast.makeText(SmsListEditActivity.this, "审核失败", Toast.LENGTH_SHORT).show();
                }
            });
