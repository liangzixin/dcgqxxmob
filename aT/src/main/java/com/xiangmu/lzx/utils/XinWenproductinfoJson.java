...
//                }
    ...
    public static XinWen_productinfo getdataFilter(String data){
        try {
            XinWen_productinfo toutiao=new XinWen_productinfo();
            JSONObject object=new JSONObject(data);
            LogUtils.e("xinwenjsonobject", object + "");
            JSONArray array = null;
            array=object.getJSONArray("Filter");//热点

            LogUtils.e("Filter----", "" + array);
            JSONObject arrayobj00=array.getJSONObject(0);
            if (! arrayobj00.isNull("totalRecords")){
                int totalRecords= arrayobj00.getInt("totalRecords");
                toutiao.setTotalRecords(totalRecords);
            ...
            JSONArray array00 = null;
            array00=arrayobj00.getJSONArray("list");
            List<XinWen_productinfo.FilterEntity> list=new ArrayList<>();
            for (int i=0;i<array00.length();i++){

                XinWen_productinfo.FilterEntity filterEntity=new XinWen_productinfo.FilterEntity();
                JSONObject arrayobj=array00.getJSONObject(i);
                if (!arrayobj.isNull("id")){
                    int skipId=arrayobj.getInt("id");
                    filterEntity.setId(skipId);
                };
                if (!arrayobj.isNull("filters")){
                    String replyCount=arrayobj.getString("filters");
                    filterEntity.setFilters(replyCount);
                };

                if (!arrayobj.isNull("note")){
            ...
            toutiao.setListfilterEntity(list);
            LogUtils.e("xinwenjson", "========" + list.get(0).getFilters());
            for (int i=0;i<toutiao.getListfilterEntity().size();i++){
                LogUtils.e("xinwenjson", i + "========" + toutiao.getListfilterEntity().get(i).getFilters());
            }
            LogUtils.e("xinwenjson", "========" + toutiao.getListfilterEntity());
            return toutiao;
        } catch (JSONException e) {
            e.printStackTrace();
        }
